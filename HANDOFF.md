...
operating record; the dated demo documents preserve earlier claims.
...
At this audit the DevKit no longer answers SSH at its last address, <devkit>.
...
| Item | Location |
|---|---|
| DevKit | `openhome@<devkit>`, Raspberry Pi 4, 8 GB, Python 3.13.5 |
...
```sh
SSH_AUTH_SOCK= ssh openhome@<devkit>   'cd ~/astral-voice/hub-v2 && ~/astral-voice/kws-venv/bin/python3 tests/run.py --full'
```
...
The creator reported submitting the application. Its proposed harness bridge and the longer
