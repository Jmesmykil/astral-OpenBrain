...
|---|---|---|
| The DevKit | `openhome@<devkit>` | Raspberry Pi 4, 4 cores, 7.7 GB, Python 3.13.5 |
| The Mac | `<mac>` | where you develop, and the network rung the device can ask |
...
ssh openhome@<devkit> 'cd ~/astral-voice/hub-v2 && ~/astral-voice/kws-venv/bin/python3 tests/run.py'
```
...
```
scp openhome@<devkit>:~/astral-voice/hub-v2/kernel/dist/astral_kernel-*.whl .
...
| `(removed)` | the answers as submitted |
| `(removed)/` | the handout and the demo transcript |
