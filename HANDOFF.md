...
Two abilities are registered on the existing account: `OpenBrain`, id 9249, category local, fourteen trigger words; and `openbraindaemon`, id 9251, category background_daemon, no trigger words, which is correct for a daemon that sees the session rather than a match. Both are active with no validation errors. OpenBrain appears in the agent's installed set; `personality_ids` on the ability record is empty and is not the assignment signal.

The earlier `jamesmykilastral` registration was deleted. The dashboard has no rename, and `openhome deploy` only creates: it refuses any name that already exists, reporting even this account's own abilities as belonging to another account. Deploy does upload code correctly for a NEW ability, which is how the daemon was published. There is no API path to updating a deployed one, and the Edit page's Upload ZIP control accepts a file, reports success and does not replace the code. OpenBrain's files were replaced through the code editor instead. `config.json` and `__init__.py` are consumed at upload and stored in the ability record rather than kept as editable files, so the platform file list is README.md, BOUNDARY.md, devkit_functions.py, requirements.txt and main.py.

The device carries one shim for both, at `local_capabilities/openbrain/`, matching BOUNDARY.md: one engine on the device, not two. Its FUNCTION_REGISTRY exposes the three entry points the daemon calls. `openhome-capability-names.txt` named the deleted `jamesmykilastral` and now names `openbrain`; the prior file is kept alongside it. The dead `astral`, `astral-daemon` and `jamesmykilastral` capability directories were renamed with a `.retired-20260913` suffix rather than deleted. Still open: platform spoken routing, human wake/room/interruption/audibility acceptance, the remaining capability/library semantic matrix, final source publication and final handoff. Read the latest local acceptance ledger for progress after this timestamp. Earlier dated sections below are historical snapshots.
...
parec capture child. All 46 protected hashes and14/160 audio levels were unchanged;
...
calls also passed. All 46 protected hashes and service PIDs were unchanged. No service
