...
#
# Then set your own names. Ability names are unique across every OpenHome account, not
# just yours, so the ones in the shipped config.json files cannot be reused: deploying
# them verbatim fails with "already exists in other users account", and that message names
# the wrong culprit. Edit unique_name and name in each config.json before uploading.
set -eu
...
cp "$SRC/__init__.py" "$SRC/main.py" "$SRC/devkit_functions.py" "$SRC/requirements.txt" \
   "$SRC/config.json" "$SRC/README.md" "$SRC/BOUNDARY.md" "$OUT/foreground/"
...
cp "$ROOT/community/astral-daemon/config.json" "$ROOT/community/astral-daemon/README.md" "$OUT/daemon/"
...
echo
echo "Set your own unique_name in each config.json before deploying: names are unique across"
echo "every OpenHome account, so the shipped ones are already taken."
