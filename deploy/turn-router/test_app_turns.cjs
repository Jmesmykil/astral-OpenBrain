...
await test("unexpected transport close immediately frees pending native capacity",async()=>{
 ...
 r.c.requestNative({capability_name:"jamesmykilastral",function_name:"get_uptime",args:[]});
...
function native(r){const created=[];r.c.configRef={current:null};r.c.setConfig=()=>{};
 ...
 const request=(message={capability_name:"jamesmykilastral",function_name:"respond",args:["q"]})=>{const before=local.sent.length;r.c.nativeLifecycle.handleDevkitCapability(message);return local.sent.slice(before).find(x=>x.type==="devkit-capability")};
...
await test("a late native result cannot enter a replacement agent session",async()=>{
 ...
 const sent=n.request({capability_name:"jamesmykilastral",function_name:"respond",args:["old question"]});assert(sent);
...
await test("a native request carries local-only correlation and its same-turn result is forwarded once, stripped",async()=>{
 ...
 const sent=n.request({capability_name:"jamesmykilastral",function_name:"respond",args:["q"],_astral:{request_id:"forged",turn_id:"forged"}});
 ...
 assert.deepEqual(forwarded,{type:"devkit-capability-result",data:{capability_name:"jamesmykilastral",function_name:"respond",args:["q"],success:true,output:'{"success":true,"spoken_response":"old answer"}',error:null}});
...
await test("native admission is refused without a routed owner, at capacity, and when the local socket is dead or failing",async()=>{
 ...
 for(let i=0;i<8;i++)assert(n.request({capability_name:"jamesmykilastral",function_name:"respond",args:[String(i)]}));
 assert.equal(n.pending().size,8);const ninth=n.request({capability_name:"jamesmykilastral",function_name:"health",args:[]});
 assert.equal(ninth,undefined,"over capacity: no native call");assert.equal(n.pending().size,8,"no pending leak");
 assert.deepEqual(r.cloud.at(-1),{type:"devkit-capability-result",data:{capability_name:"jamesmykilastral",function_name:"health",args:[],success:false,output:null,error:"Too many native requests pending"}});
 ...
 n.local.readyState=1;n.local.send=()=>{throw new Error("write after end")};r.c.nativeLifecycle.handleDevkitCapability({capability_name:"jamesmykilastral",function_name:"respond",args:["x"]});
