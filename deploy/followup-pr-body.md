...
The engine source and tests now ship next to main.py, in hub/. main.py is generated from them (python3 hub/build_ability.py) and the artifact parity test proves the shipped file answers every phrase the way the source does. the golden suite pins 155 phrases byte for byte, the ones it must stay silent on included. the hardening suite fuzzes four thousand utterances and rechecks every result a second way.
