...
`requirements.txt`, which the platform installs on the DevKit exactly as it installs any
...
bargain this ability is built on: it answers what it can answer exactly, instantly, on
...
3. **Neither** — and then the ability *says so out loud*: "The Astral engine is not
   ...
   which would leave a broken install looking exactly like a working one forever.
