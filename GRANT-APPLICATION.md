...
The rest of my work is the same idea at a larger scale, and most of it is further along than this device. The Astral Brain Engine (607 commits) is the harness: a model-agnostic orchestration layer with a delegation hierarchy, persistent memory, budgets and a task board, and it is what routed the work on this project. Strata is Astral MECH, a language comprehension engine written in Julia with Ada SPARK proofs and Rust lanes, no model weights and no sampling: it reads a sentence into a typed role graph, keeps what you tell it with the source attached, gives byte-identical output for the same input, and says so when it cannot read something (436 commits, a signed macOS app). Slate is its math lane and runs on the DevKit today as the kernel service. Around the engine sits an ecosystem I built: Q OS, a windowed desktop environment for the Nintendo Switch that replaces the home menu and is verified on hardware (public); Void IDE and Void CLI, a native macOS editor in Swift and a Rust terminal that both speak to the engine; Astral Workspace and the Swift packages under it; the ASCII glyph-grid render kernel with hosts for Unity 6, Blender and Tessera (3D models in, sprite sheets out, no image model in the path); sCAN, a crypto research desktop with a Rust gateway and PostgreSQL (179 commits); SlimeShot, a Unity game that is close to done; MoBA, a server-authoritative online game that is close to mechanically done (579 commits); soloDOLO, an animated series in production; SourceBound, a 2D RPG that teaches coding; Filament, a hardware-in-the-loop firmware workbench; and EMULive, a living Game Boy emulator. The pattern across all of it is local first, with the cloud as a choice and not a default.
