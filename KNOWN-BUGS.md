...
## Current open acceptance — September 13, 2026, 06:03 UTC
...
- The hosted ability record (v1) lacks the local corpus hook and heard shim. The unshipped hook
  ...
  foreground calls remain available. The dispatcher in this repository, not yet installed
  on the development device, reads registration names from the device's
  `openhome-capability-names.txt`, so a renamed registration needs its line there.
...
are described above. Clock times in these notes are the device's local time unless marked UTC.
...
**Pages laid out in columns were read straight across.** A reference volume sets a body column beside
...
recogniser it was fixable here. The evidence is in the private acceptance records.

**The column repair covers 95.4 percent of it.** Measured across all 65 volumes of the reference library rather than
...
and are written down in the private acceptance records, because each measured
...
voice and the speech model they picked with the defaults, so the device sounded or heard wrong
with the symptom and the cause in different places.

**Still open.** The reference library's most heavily designed spreads, where display lettering is set through
...
is now measured, in the private acceptance records, but it is log
analysis, not a labelled corpus with a person in the room. Earlier dated sections below are
historical snapshots.

## Current runtime verification — September 6, 2026, 06:02 UTC
...
The schema26/source rollback is retained at /home/openhome/astral-voice/platform-hardening/library27-v1/rollback. Local acceptance records include the deployment and full-device receipts and a pre-mortem review. Source commits have not been pushed. Earlier dated snapshots below are historical and may describe issues subsequently resolved.

## Previous runtime verification — September 6, 2026, 05:03 UTC
...
## Current account and repeat deployment — September 6, 2026, 03:50 UTC

The existing account key works. Astral is registered under the account's own name, installed on the DevKit and assigned to the existing Astral agent. The account's other abilities and agents were preserved. Three actual Node/Python/shim/hub requests through the registered name pass. This is not a platform spoken-turn receipt.
...
## Actual app control — September 6, 2026, 02:50 UTC
...
## Installation reconciliation — September 6, 2026, 02:41 UTC
...
Existing device-key SDK access returns 200 for the Astral agent. Its explicit matching_capabilities list is empty; this is separate from the globally installed/enabled abilities. Management-list access with the device key returns 401 and needs its session mode. The saved draft's resume remains blank. No account upload/assignment or hosted inference occurred. No new account or GPT-app sign-in is needed.
...
## Historical deployment — September 6, 2026, 02:11 UTC
...
The earlier one-hour observer completed 183/183 native answers with zero detector issues, before this library migration. It does not prove human audibility or a final library 26 acoustic soak. No new account or GPT-app login is needed. Astral ability registration/assignment remains unfinished, with its draft opening blank. Human voice/app acceptance, remaining coverage and final publication remain open. Later local commits are not yet pushed; published kernel 2.2.3 bytes are unchanged.

The earlier dated snapshots below retain their historical counts and pending statements.

The source Python handbook contains malformed examples; extraction preserves original author content rather than silently correcting it.

## Historical deployment snapshot — September 5, 2026, 20:19 UTC
...
finished successfully; the voice service started at 20:13:19 UTC (PID 19661), the shared
...
A pre-mortem pass reproduced and repaired 22 findings across library
...
match the installed artifact. The package validates with the CLI carrying the DevKit validator fix.
...
verification. A power cycle restored OpenHome's saved speaker 14% and microphone 160%,
...
interruption while thinking/speaking; app-slider behavior and audibility. The retired
trained wake head is
...
the private audit record for the 2026-09-05 pre-mortem pass.

## Earlier verified baseline — September 5, approximately 11:15 UTC

The current deployment and verification state is in the first section of this file. The
sections below this notice are a chronological record; their early “hey mycroft” and forced-mixer
...
restart error reporting have been repaired and deployed. The recovered encyclopedia
...
answers and eight device calls. Mac lint was checked separately and passed. These
software results do not close physical acceptance.
...
The reader sometimes hands the question back rearranged: "what are the laws of
...
open ones poorly. That is a question for the reader, not for routing, and it is still open.
...
06:00 to 06:20 on the DevKit in a quiet room with nobody speaking: 20 minutes, one
...
**2. The reader's corpus contains web-development text.** The reader answered "when did the
...
the encyclopedia had articles on each. The cause was an optional article written `(?:a|an|the)?`
...
**4. The encyclopedia has no page numbers, and cannot get them from this download.** INVESTIGATED
...
**5. Encyclopedia volume 29 is unreadable.** Its gzip is corrupt at the source, not
...
fix of the day was on the card and none of it was running. The deploy now restarts a
loop it finds running. The restart branch is committed but has not yet been exercised by a real
deploy, the one run since found the loop already stopped.

**Fixed and proved on the device** (an evidence script run there, 22 of 23 sentences
...
**Open, and not fixable without recordings of a real voice.** The phrase recogniser wakes on
...
quiet. The lasting fix is a trained wake model from real recorded takes (`wake_takes.py`).
...
Peak-matched chimes sounded quiet by ear, a short transient at the same peak as a
...
that is still set by ear is the master.

**Later the same day, after hands-on tests.**
...
**Later still, 2026-09-03, the volume levels.**
...
- **Power was cut at about 13:28; clean boot at 15:33, 37.9 °C.** The voice loop was not
  ...
  38 percent at every boot; the loop applies the remembered master once, at boot,
...
## 2026-09-04, early morning, the boot race, the app's slider, follow-ups
...
## 2026-09-04, 08:00, the sweeps: 645 utterances, 88 flows, one voice pass

Fixed and deployed that morning, each with a check proved red first: a filler word in
...
## 2026-09-04, mid-morning

- **Hardware:** the DevKit's audio cable connections can need reseating from time
  to time. That is a candidate cause for the deaf starts and the
...
- **The library:** 65 volumes of a reference library went onto the reference shelf and
  indexed in 116 s: 397,706 entries from 191 sources, a 660 MB index. Two volumes had no
  text layer; they need OCR. The first library question after the index grew took 50 s
  cold.
- **Shipping v2:** the kernel wheel is a GitHub release (v2.2.0) and requirements.txt
  ...
  (unique_name, matching_hotwords, category local). OpenHome's CLI validator applied the
  capability-worker rules to devkit_functions.py, which rejects every valid DevKit
  ability including OpenHome's own devkit-stats template; the fix is on a fork's main
  branch (not yet published to npm). `openhome login`, deploy, assign and trigger
  registration are the account holder's commands.


## 2026-09-04, midday, the sweep fixes merged and the loop shipped

Three review passes ran that morning, each in its own worktree, each defect reproduced
...
- **ssh from the Mac hung again** at the public-key step: the launchd agent socket. With
  `ASTRAL_SSH_KEY` set, the deploy script uses that key alone and hides the agent;
  anything ad hoc can do the same (`SSH_AUTH_SOCK= ssh -i <key> -o IdentitiesOnly=yes …`).
...
- **The rule:** all volume is mastered on our side and handed to the OpenHome app
...
- **What changed.** The loop no longer sets the speaker or the microphone at boot and the
  ...
  wake was measured deaf at 30 ("open brain, run the demo" at peak 112 that
  morning); a value chosen later, by them or by the owner, is never overwritten. The
...
- **Mastered.** The astral pack's cues were levelled at play time to one RMS with a fixed
  ...
  between drops), was approved by ear at 10:45 (after the drop was
...
- **Still not proven by voice since the merges:** wake to whisper to answer to speech,
  ...
  test phrases through the device's own speaker and needs an explicit go-ahead.

## 2026-09-04, afternoon, the ship sweep

The goal: no gaps or conflicts, ready to ship. Every
...
- **What the device says is in the log now** (`[said] …` beside `[heard] …`). Until today
  ...
  back, and neither could anyone reading the log.
...
- **The device rebooted by itself at 14:58** (their node server's journal: "Setting volume
  ...
  the microphone level, and has answered since. What this means: after every
...
- **The second voice pass, at sink 65, worked where it was read:** "what time is it" was
  ...
  the device; its full record, kept on the device, had not been read back
...
- **The second voice pass, read in full (13 turns through the device's own speaker at sink 65).**
  ...
  a riddle", so the timer rang two minutes later. Two turns were contaminated by someone
...
- **The ability is ready and not yet deployed.** `openhome validate community/astral` passes
  with the CLI's validator fix. `openhome login`, `deploy` and `assign` are the account
  holder's commands.
